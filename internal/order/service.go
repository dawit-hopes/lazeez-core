...
type OrderService interface {
	...
	CreateClient(ctx context.Context, order OrderInput) error
...
func (s *orderService) CreateClient(ctx context.Context, order OrderInput) error {
	...
	if itemsTotal < order.Total-epsilon || itemsTotal > order.Total+epsilon {
		...
		return common.ErrOrderTotalMismatch
	}
...
func (s *orderService) UpdateBranch(ctx context.Context, id string, input OrderUpdateInput, branchID string) error {
	...
	return s.orderRepository.UpdateStatus(ctx, id, input.OrderStatus)
}
